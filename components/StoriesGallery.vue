...
export default {
  ...
  data() {
    return {
      ...
      currentImageIndex: [],
      intervals: [],
      animationStarted: false, // Nuova variabile per assicurarsi che l'animazione non parta più volte
    };
  },
  mounted() {
    if (process.client) {
  ...
  methods: {
    async fetchPostsData() {
      try {
        ...
        this.$nextTick(() => {
          // Esegui l'animazione dopo che le immagini sono state caricate
          if (!this.animationStarted) {
            this.animateGalleryImages();
            this.animationStarted = true; // Assicura che l'animazione non venga ripetuta
          }
        });
